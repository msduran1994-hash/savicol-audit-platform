...
// Proxy seguro para Anthropic — la API key nunca se expone al browser
...
export async function POST(req: NextRequest) {
  try {
    ...
    const prompt = [
      "Eres un auditor de bioseguridad avícola con experiencia en granjas colombianas de Savicol.",
      ...
      "- Máximo 80 palabras en un solo párrafo.",
      "- Lenguaje profesional, claro, humano y accionable.",
      "- Incluye: acción concreta, responsable sugerido, plazo estimado.",
      "- Específico para granjas avícolas colombianas.",
      "- Comienza directamente con la acción. Sin introducciones.",
    ].filter(Boolean).join("\n");
    ...
    if (!response.ok) {
      console.error("[AI Route] Anthropic error:", response.status);
      return NextResponse.json(
