"use client";
// ═══════════════════════════════════════════════════════════════════════════════
// /activar?token=... · Activación de cuenta desde invitación
// ═══════════════════════════════════════════════════════════════════════════════
import { useEffect, useState, Suspense } from "react";
import { useSearchParams, useRouter } from "next/navigation";
import Link from "next/link";
import { Shield, Lock, Eye, EyeOff, AlertCircle, CheckCircle2, Loader2, Mail, UserCircle2 } from "lucide-react";
import { validateInvitationToken, acceptInvitation } from "@/hooks/useInvitations";

function ActivarPageContent() {
  const router = useRouter();
  const search = useSearchParams();
  const token  = search.get("token") ?? "";

  const [loading, setLoading]     = useState(true);
  const [validation, setValidation] = useState<{ email: string; name: string; role: string; expiresAt: string } | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const v = await validateInvitationToken(token);
        if (cancel) return;
        setValidation(v);
        setName(v.name);
      } catch (e: any) {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    if (!name.trim())                { setSubmitError("El nombre es obligatorio"); return; }
    ...
    try {
      ...
      setSubmitError(e?.response?.data?.message ?? e?.message ?? "Error al activar la cuenta");
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <div className="w-full max-w-md">
        ...
        <div className="flex items-center justify-center gap-3 mb-8">
          ...
          <div>
            ...
            <p className="text-xs text-[#94A3B8] mt-0.5">Activación de cuenta</p>
          </div>
        </div>
        ...
        <div className="bg-[#0D1526] border border-[#1E2D4A] rounded-2xl p-6 shadow-2xl">
          {loading ? (
            <div className="py-12 text-center">
              ...
              <p className="text-[#94A3B8] text-sm">Validando invitación...</p>
            </div>
          ) : validationError ? (
            <div className="py-8 text-center">
              ...
              <p className="text-white font-semibold mb-2">Invitación no válida</p>
            ...
            <div className="py-8 text-center">
              ...
              <p className="text-white font-semibold mb-2">¡Cuenta activada!</p>
            ...
            <>
              <div className="mb-6">
                <p className="text-xs text-[#94A3B8] mb-1">Invitado:</p>
                <div className="flex items-center gap-2 text-white text-sm">
                  <Mail className="w-3.5 h-3.5 text-amber-400"/>
                  {validation?.email}
                </div>
                <p className="text-[10px] text-[#475569] mt-2">
                  Rol asignado: <span className="text-amber-400 font-semibold">{validation?.role}</span>
                </p>
                {validation?.expiresAt && (
                  <p className="text-[10px] text-[#475569] mt-1">
                    Esta invitación expira el <span className="text-white">{new Date(validation.expiresAt).toLocaleString("es-CO", { dateStyle: "medium", timeStyle: "short" })}</span>
                  </p>
                )}
              </div>
              ...
              <form onSubmit={onSubmit} className="space-y-4">
                <div>
                  <label className="text-xs text-[#94A3B8] block mb-1.5">Tu nombre completo</label>
                  <div className="relative">
                    <UserCircle2 className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-[#475569]"/>
                ...
                <div>
                  <label className="text-xs text-[#94A3B8] block mb-1.5">Crear contraseña</label>
                  <div className="relative">
                ...
                <p className="text-[10px] text-[#475569] text-center pt-2">
                  Al activar, aceptas las políticas de uso y seguridad de Savicol.
                </p>
              </form>
            </>
          )}
